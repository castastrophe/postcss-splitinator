...
module.exports = (opts = {}) => {
  opts.noFlatVariables = opts.noFlatVariables ?? false;
  opts.noSelectors = opts.noSelectors ?? false;
  ...
  return {
    ...
    prepare() {
      ...
      return {
        Rule(container, { Rule }) {
          if (container.name !== 'container') return;
          ...
          container.walkDecls(decl => {
            ...
            selectors.forEach(selector => {
              const variableName = typeof options.getName === 'function' ? options.getName(selector, decl.prop) : ((selector, prop) => {
                selector = selector.replace(/^:where\((.*?)\)$/, '$1');

                // This regex is designed to pull spectrum-ActionButton out of a selector
                const baseSelectorMatch = selector.match(/^\.([a-z]+-[\A-Z][^-. ]+)/);
              ...
              if (!options.noFlatVariables) rule.append(newDecl);
